...
Let's say, we want to share a certain time series with the world, so we create an API
...
write the function that we will apply to create the API
```{r}
...
<!-- TODO: get the param setting of start and end date to work -->

```{r}
# to actually create an API, need to run this (& functions above) in a file called plumber.R in the RStudio IDE
...
# with: curl -X 'GET' \ 'http://127.0.0.1:41825/filtered_data?start=2010' \ -H 'accept: */*'
