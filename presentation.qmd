...
<!-- 
Speaker Notes:
  API = standardising the way in which your computer talks to another computer (=server)
  somehow, we need to tell the other computer where we want to go, and what we want to get, this is done with this link:

  Analogy:
  - so let's think about APIs in the way we have been trying to get data before: 
  - we (the client) want data from BFS's database (the server)
  - this is difficult to navigate, so we need to click around the website, search for things, and look around to get what we need.
  - API makes this searching and clicking around process easier, because it's an automated way of machine to machine communication
  - but how? because of protocols


  API Protocol:
  - protocol means explicit way of behaving, we as humans have implicit protocol too (like how to behave), but machines have explicit protocols, you can see this with http (how the internet works)
  - HTTP has 4 main verbs: GET, POST, DELETE, PUT

 -->
  <!-- TODO: more research on http protocol (& difference in REST) -->
...
- client makes a request to the server, specifying the following information:
  ...
  - how (is the data represented)? -> params (after special char. used for this: ? &)
 -->
...
<!-- TODO: rewrite bfs_get_data simply, to work for our cause: spits out url and data-->
...
<!-- TODO: why is this not containing the actual data too?

figure out how to proceed here? show browser data after all?
  -->
![](img/browser.png)
...
<!-- TODO: give them function template -->
